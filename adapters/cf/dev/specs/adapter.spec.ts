...
function createMockCloudflareBindings() {
...
describe('createCloudflareVectorizeIntegration', () => {
  ...
  describe('getConfigExtension', () => {
    test('should return config with pool configurations', () => {
      ...
      const config = { mainPool: { dims: 384 }, secondaryPool: { dims: 768 } }

      const { adapter } = createCloudflareVectorizeIntegration(config, { vectorize: mockVectorize })
      const extension = adapter.getConfigExtension({} as any)

      expect(extension.custom?._cfVectorizeAdapter).toBe(true)
      expect(extension.custom?._poolConfigs).toEqual(config)
    })
  })

  describe('storeEmbedding', () => {
    test('should convert Float32Array to regular array', async () => {
      const mockBindings = createMockCloudflareBindings()
      ...
      expect(mockBindings.vectorize.upsert).toHaveBeenCalledWith([
        {
          id: 'test-id',
    ...
    test('should inject vectorize binding into context', async () => {
      ...
      expect(mockPayload.context.vectorize).toBe(mockBindings.vectorize)
    })
  })
  ...
  describe('deleteEmbeddings', () => {
    test('should query with correct where clause', async () => {
      ...
      expect(mockBindings.vectorize.query).toHaveBeenCalled()
      const queryCall = mockBindings.vectorize.query.mock.calls[0]
    ...
    test('should delete matching vectors', async () => {
      ...
      const storage = mockBindings.vectorize.__getStorage()
      storage.set('vec-1', {
        id: 'vec-1',
      ...
      expect(mockBindings.vectorize.delete).toHaveBeenCalledWith(['vec-1', 'vec-2'])
    })
    ...
    test('should handle empty results gracefully', async () => {
      ...
      expect(mockBindings.vectorize.delete).not.toHaveBeenCalled()
    })

    test('should handle query errors', async () => {
      const mockBindings = createMockCloudflareBindings()
      mockBindings.vectorize.query = vi.fn().mockRejectedValue(new Error('Query failed'))
